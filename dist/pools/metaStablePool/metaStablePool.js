...
class MetaStablePool {
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        if (pairType !== types_1.PairTypes.BptToToken) {
            tokenIndexIn = this.tokens.findIndex(t => address_1.getAddress(t.address) === address_1.getAddress(tokenIn));
            if (tokenIndexIn < 0)
                throw 'Pool does not contain tokenIn';
            tI = this.tokens[tokenIndexIn];
            // balanceIn = tI.balance;
            balanceIn = bmath_1.bnum(tI.balance).times(bmath_1.bnum(tI.priceRate));
            decimalsIn = tI.decimals;
            tokenInPriceRate = bmath_1.bnum(tI.priceRate);
        }
        if (pairType !== types_1.PairTypes.TokenToBpt) {
            tokenIndexOut = this.tokens.findIndex(t => address_1.getAddress(t.address) === address_1.getAddress(tokenOut));
            if (tokenIndexOut < 0)
                throw 'Pool does not contain tokenOut';
            tO = this.tokens[tokenIndexOut];
            // balanceOut = tO.balance;
            balanceOut = bmath_1.bnum(tO.balance).times(bmath_1.bnum(tO.priceRate));
    ...
    _exactTokenInForTokenOut(poolPairData, amount) {
        ...
        let amt = metaStableMath_1._exactTokenInForTokenOut(amount.times(poolPairData.tokenInPriceRate), poolPairData).dp(poolPairData.decimalsOut, 1);
    ...
    _tokenInForExactTokenOut(poolPairData, amount) {
        ...
        let amt = metaStableMath_1._tokenInForExactTokenOut(amount.times(poolPairData.tokenOutPriceRate), poolPairData).dp(poolPairData.decimalsIn, 0);
    ...
    _spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        ...
        let result = metaStableMath_1._spotPriceAfterSwapExactTokenInForTokenOut(amountConverted, poolPairData);
        return result;
    }
    _spotPriceAfterSwapExactTokenInForBPTOut(poolPairData, amount) {
        return metaStableMath_1._spotPriceAfterSwapExactTokenInForBPTOut(amount, poolPairData);
    }
    _spotPriceAfterSwapExactBPTInForTokenOut(poolPairData, amount) {
        return metaStableMath_1._spotPriceAfterSwapExactBPTInForTokenOut(amount, poolPairData);
    }
    _spotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        ...
        let result = metaStableMath_1._spotPriceAfterSwapTokenInForExactTokenOut(amountConverted, poolPairData);
        return result;
    }
    _spotPriceAfterSwapTokenInForExactBPTOut(poolPairData, amount) {
        return metaStableMath_1._spotPriceAfterSwapTokenInForExactBPTOut(amount, poolPairData);
    }
    _spotPriceAfterSwapBPTInForExactTokenOut(poolPairData, amount) {
        return metaStableMath_1._spotPriceAfterSwapBPTInForExactTokenOut(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        return metaStableMath_1._derivativeSpotPriceAfterSwapExactTokenInForTokenOut(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapExactTokenInForBPTOut(poolPairData, amount) {
        return metaStableMath_1._derivativeSpotPriceAfterSwapExactTokenInForBPTOut(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapExactBPTInForTokenOut(poolPairData, amount) {
        return metaStableMath_1._derivativeSpotPriceAfterSwapExactBPTInForTokenOut(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        return metaStableMath_1._derivativeSpotPriceAfterSwapTokenInForExactTokenOut(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapTokenInForExactBPTOut(poolPairData, amount) {
        return metaStableMath_1._derivativeSpotPriceAfterSwapTokenInForExactBPTOut(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapBPTInForExactTokenOut(poolPairData, amount) {
        return metaStableMath_1._derivativeSpotPriceAfterSwapBPTInForExactTokenOut(amount, poolPairData);
    }
    ...
    _evmexactBPTInForTokenOut(poolPairData, amount) {
        try {
            ...
            const bptTotalSupplyScaled = bmath_1.scale(poolPairData.balanceIn, 18);
