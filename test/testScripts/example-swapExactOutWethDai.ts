...
const amountOut = new BigNumber('1e18'); // 1 DAI, Always pay attention to Token Decimals. i.e. In this case DAI has 18 decimals.
const tokenIn = WETH;
const tokenOut = DAI;
...
async function swapExactIn() {
    ...
    */
    ...
    console.log(`Retrieving IPFS Pools...`);
    let ipfsPools = await ipfs.getAllPublicSwapPools(
        `balancer-team-bucket.storage.fleek.co/balancer-exchange/pools?cb=${Math.random() *
            10000000000000000}`,
        'ipns'
    );
