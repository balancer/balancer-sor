...
export function getOutputAmountSwapForPath(
    ...
    if (poolPairData.length == 1) {
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            ...
            return getOutputAmountSwap(pools[1], swapType, outputAmountSwap1);
        } else {
            ...
            return getOutputAmountSwap(pools[0], swapType, outputAmountSwap2);
        }
    } else {
...
export function getSpotPriceAfterSwapForPath(
    ...
    if (poolPairData.length == 1) {
        return getSpotPriceAfterSwap(pools[0], swapType, amount);
    } else if (poolPairData.length == 2) {
        if (swapType === SwapTypes.SwapExactIn) {
            let outputAmountSwap1 = getOutputAmountSwap(
                pools[0],
...
export function getSpotPriceAfterSwap(
    ...
    if (swapType === SwapTypes.SwapExactIn) {
        ...
        if (amount.gte(pool.poolPairData.balanceOut)) return bnum('Infinity');
    }
    if (swapType === SwapTypes.SwapExactIn) {
        if (pairType === PairTypes.TokenToToken) {
            return pool._spotPriceAfterSwapExactTokenInForTokenOut(amount);
        } else if (pairType === PairTypes.TokenToBpt) {
            return pool._spotPriceAfterSwapExactTokenInForBPTOut(amount);
        } else if (pairType === PairTypes.BptToToken) {
            return pool._spotPriceAfterSwapExactBPTInForTokenOut(amount);
        }
    } else {
        if (pairType === PairTypes.TokenToToken) {
            return pool._spotPriceAfterSwapTokenInForExactTokenOut(amount);
        } else if (pairType === PairTypes.TokenToBpt) {
            return pool._spotPriceAfterSwapTokenInForExactBPTOut(amount);
        } else if (pairType === PairTypes.BptToToken) {
            return pool._spotPriceAfterSwapBPTInForExactTokenOut(amount);
        }
    }
}
...
export function getDerivativeSpotPriceAfterSwapForPath(
    ...
    let poolPairData = path.poolPairData;
    if (poolPairData.length == 1) {
        return getDerivativeSpotPriceAfterSwap(path.pools[0], swapType, amount);
    } else if (poolPairData.length == 2) {
        if (swapType === SwapTypes.SwapExactIn) {
            let outputAmountSwap1 = getOutputAmountSwap(
                path.pools[0],
            ...
            let SPaS1 = getSpotPriceAfterSwap(path.pools[0], swapType, amount);
            ...
            let SPaS2 = getSpotPriceAfterSwap(path.pools[1], swapType, amount);
            let dSPaS1 = getDerivativeSpotPriceAfterSwap(
                path.pools[0],
                swapType,
                outputAmountSwap2
...
export function EVMgetOutputAmountSwap(
    ...
    if (swapType === SwapTypes.SwapExactIn) {
        ...
        if (pool.poolType === PoolTypes.Weighted) {
            if (pairType === PairTypes.TokenToToken) {
                returnAmount = pool._evmoutGivenIn(amount);
                ...
                returnAmount = pool._evmexactTokenInForBPTOut(amount);
                // TODO: scaling down may not be necessary since we have to
                // scale it up anyways for the swap info later?
                returnAmount = scale(returnAmount, -18); // BPT is always 18 decimals
            } else if (pairType === PairTypes.BptToToken) {
                returnAmount = pool._evmexactBPTInForTokenOut(amount);
                // TODO: scaling down may not be necessary since we have to
                // scale it up anyways for the swap info later?
                returnAmount = scale(returnAmount, -decimalsOut);
            }
            ...
            returnAmount = getOutputAmountSwap(pool, swapType, amount);
        }
    } else {
        // TODO we will be able to remove pooltype check once all EVM maths is available
        if (pool.poolType === PoolTypes.Weighted) {
            if (pairType === PairTypes.TokenToToken) {
                returnAmount = pool._evminGivenOut(amount);
                // TODO: scaling down may not be necessary since we have to
                // scale it up anyways for the swap info later?
                returnAmount = scale(returnAmount, -decimalsIn);
            } else if (pairType === PairTypes.TokenToBpt) {
                returnAmount = pool._evmtokenInForExactBPTOut(amount);
                // TODO: scaling down may not be necessary since we have to
                // scale it up anyways for the swap info later?
                returnAmount = scale(returnAmount, -decimalsIn);
            } else if (pairType === PairTypes.BptToToken) {
                returnAmount = pool._evmbptInForExactTokenOut(amount);
                // TODO: scaling down may not be necessary since we have to
                // scale it up anyways for the swap info later?
                returnAmount = scale(returnAmount, -18); // BPT is always 18 decimals
            }
