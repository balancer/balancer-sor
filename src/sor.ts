...
export const smartOrderRouterMultiHopNEW = (
    pools: any[],
    paths: Path[],
    swapType: string,
    totalSwapAmount: BigNumber,
    ...
    pricesOfInterest = calculateBestPathIdsForPricesOfInterestNEW(
        pricesOfInterest,
        maxPools
    );
...
export function processEpsOfInterestMultiHopNEW(
    sortedPaths: Path[],
    swapType: string,
    maxPools: number
): EffectivePrice[] {
...
function calculateBestPathIdsForPricesOfInterestNEW(
    pricesOfInterest: Price[],
    maxPools: number
): Price[] {
    let bestPathsIds = [];
